...
const SalaryReportTable = () => {
  ...
  if (Array.isArray(data) && data.length > 0 && Array.isArray(data[0].monthlySalaryStatus)) {
    ...
    const months = data[0].monthlySalaryStatus.map((monthStatus) => ({
      ...
      align: "center",
      renderCell: (params) => {
        const label = params.row.monthlySalaryStatus.find((status) => status.month === monthStatus.month)?.status;

        return label ? <CustomChip label={label} status={label.toLowerCase()} /> : null;
      },
    }));
  ...
  return (
    <>
      <Paper sx={{ display: "flex", alignItems: "center" }}>
        <Typography style={{ marginRight: "18px", fontSize: "1.1rem", fontWeight: "500" }}>
